...
WDIO + Applitools NML — iOS against a **local Appium server** (multi-device variant),
no cloud vendor. Runs `analyticsX_nml_multidevice.ios.test.js`, which uses Applitools'
`Configuration.addMultiDeviceTarget(...)` to render the same checkpoints across
multiple simulated iPhone form factors from a single local Appium session.
